...
مشروع **SmartReaderPro** هو عبارة عن أداة متطورة (بنسخة للباحثين) مصممة لتحليل واستخراج النصوص والبيانات الأكاديمية المعقدة من المستندات (مثل الصور وملفات PDF). يقوم المشروع بتحويل محتوى الصفحات إلى هيكل بيانات منظم ودقيق جداً (بصيغة مخصصة تُسمى `.jsoni`)، مع استخراج عناصر دقيقة كالحواشي السفلية، الفهارس، التوصيف الدلالي (Semantic Tags)، وتحديد بنية الأبحاث كالمقدمات والنتائج.
...
*   **تكامل الذكاء الاصطناعي (AI Integration):**
    *   يعتمد المشروع بشكل أساسي على نماذج متقدمة مثل **Gemini 1.5 Pro** و **Gemini 2.5 Flash** لكتابة "Prompts" معقدة تقوم باستخراج النصوص، الإحداثيات (Bounding Boxes)، تحليل حالة الصفحة، وفهم السياق الأكاديمي.
...
*   **هيكلة البيانات:**
    *   **JSON (بامتداد .jsoni):** هو العصب الأساسي للبيانات في هذا المشروع، حيث تُحفظ جميع المخرجات (نصوص، إحداثيات، بيانات وصفية للكتب، وعناصر الأبحاث) داخل ملفات مهيكلة بدقة شديدة لتسهيل مراجعتها وتصنيفها لاحقاً.
